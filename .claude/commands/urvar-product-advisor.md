...
**Note:** Product knowledge is retrieved automatically at query time from `RAG/docs/` via vector search. You do not need to manually read files — the most relevant sections are already in your context. If you need more detail, use `/update-knowledge-base` to check the doc content.
...
- Market research or pricing comparisons (use `/agent-routing` for routing reference)
...
- You are about to state a dosage not present in the retrieved RAG context → stop, state you need to verify
...
**Expected output:**
Use the per-katha rate from the retrieved context (from `RAG/docs/crop-guide.md`), mention it's a basal dose applied before transplanting, note 1 katha = 720 sq ft.
...
| Inventing a dosage | No RAG context retrieved | Ask the user to rephrase so retrieval picks up the right doc section |
...
Product knowledge is automatically retrieved from:
- `RAG/docs/products.md` — full nutrient analysis, application rates, storage
- `RAG/docs/crop-guide.md` — crop-specific programs and deficiency quick-reference table
- `RAG/docs/disease-guide.md` — disease/pest diagnosis and treatment

To update this knowledge, see `/update-knowledge-base`.

---

## Notes / Limitations

- Pricing is a template in `RAG/docs/` — actual MRP values may not be filled in; never invent prices
- Pack sizes are fixed — do not suggest sizes Urvar does not offer
- If RAG retrieval returns empty context (Voyage AI unavailable), fall back to the 8-product table in this file
- Crop programs in RAG docs use kathas as the unit — always convert for users who ask in acres or sq ft
